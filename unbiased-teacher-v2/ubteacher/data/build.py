...
#Anado que reciba reduce_unlabel_data, unlabel_percent_use para reducir el numero de no etiquetados (para hacer pruebas)
def divide_label_unlabel(
    dataset_dicts, SupPercent, random_data_seed, random_data_seed_path,reduce_unlabel_data, unlabel_percent_use
):
    ...
    #Reduzco el numero de no etiquetadas si es necesario
    if reduce_unlabel_data:
        num_unlabel=int((num_all-num_label)*unlabel_percent_use*0.01)
        all_idx=set(range(num_all))
        unlabeled_idx=all_idx.difference(labeled_idx)
        unlabeled_idx=set(random.sample(list(unlabeled_idx), num_unlabel)) 

    if not reduce_unlabel_data:
        ...
        #Si reducimos el porcentaje de no supervisados
        for i in range(len(dataset_dicts)):
            if i in labeled_idx:
                label_dicts.append(dataset_dicts[i])
...
def build_detection_semisup_train_loader(cfg, mapper=None):
    ...
    label_dicts, unlabel_dicts = divide_label_unlabel(
        ...
        cfg.DATALOADER.REDUCE_UNLABEL_DATA,
        cfg.DATALOADER.UNLABEL_PERCENT_USE,
    )
...
def build_detection_pcb_loader(cfg,label_dataset,mapper=None):
    if mapper is None:
        mapper = DatasetMapper(cfg, True)
    ...
    sampler_name = cfg.DATALOADER.SAMPLER_TRAIN
    logger = logging.getLogger(__name__)
    logger.info("Using training sampler for pcb{}".format(sampler_name))
    if sampler_name == "TrainingSampler":
        ...
        raise ValueError("Unknown training sampler: {}".format(sampler_name))
    return detectron2.data.build.build_detection_test_loader(dataset=label_dataset, mapper=mapper, num_workers=cfg.DATALOADER.NUM_WORKERS)
...
def build_detection_semisup_train_loader_two_crops(cfg, mapper=None):
    ...
    #Sera necesario para el pcb
    label_dataset_noAugmentations=label_dataset
    unlabel_dataset_noAugmentations=unlabel_dataset
    ...
    return build_semisup_batch_data_loader_two_crop(
        ...
        aspect_ratio_grouping=cfg.DATALOADER.ASPECT_RATIO_GROUPING,
        num_workers=cfg.DATALOADER.NUM_WORKERS,
    ),label_dataset_noAugmentations,unlabel_dataset_noAugmentations
