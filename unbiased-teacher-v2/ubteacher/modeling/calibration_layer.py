...
from ubteacher.modeling.archs import resnet101
...
class PrototypicalCalibrationBlock:
    ...
    def __init__(self, cfg, data_loader):
        ...
        #O dataloader conten os datos etiquetados sen augmentations
    ...
    def build_prototypes(self):
        ...
        for data in self.dataloader.dataset:
            img = cv2.imread(data['file_name'])  # BGR
            img_h, img_w = img.shape[0], img.shape[1]
            ratio = img_h / data['instances'].image_size[0]
            data['instances'].gt_boxes.tensor = data['instances'].gt_boxes.tensor * ratio
            boxes=data["instances"].gt_boxes.to(self.device)

            # extract roi features
            features = self.extract_roi_features(img, [boxes])
            all_features.append(features.cpu().data)

            gt_classes = [data['instances'].gt_classes]
    ...
    def execute_calibration(self, inputs, dts):
        ...
        ileft = (dts.scores > self.cfg.TEST.PCB_UPPER).sum()
        iright = (dts.scores > self.cfg.TEST.PCB_LOWER).sum()
        assert ileft <= iright
        boxes = [dts.pred_boxes[ileft:iright]]

        features = self.extract_roi_features(img, boxes)

        for i in range(ileft, iright):
            tmp_class = int(dts.pred_classes[i])
            ...
            dts.scores[i] = dts.scores[i] * self.alpha + tmp_cos * (1 - self.alpha)
        return dts
